...
public final class PlayersStore {
    ...
    public PlayersStore(Path filePath) {
        this.filePath = filePath;
    }

    public Map<String, PlayerVipState> load() {
        synchronized (lock) {
            ensureExists();
            try {
                byte[] bytes = Files.readAllBytes(filePath);
                Root root = MAPPER.readValue(bytes, Root.class);
                if (root.players == null) root.players = new HashMap<>();
                return root.players;
            } catch (IOException e) {
    ...
    public void save(Map<String, PlayerVipState> players) {
        synchronized (lock) {
            ensureExists();
            Root root = new Root();
            root.version = 1;
            root.players = (players == null) ? new HashMap<>() : players;
    ...
    private static final class Root {
        public int version;
        public Map<String, PlayerVipState> players;
    }
}
